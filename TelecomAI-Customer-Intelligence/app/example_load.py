...
def load_pipeline():
    if not MODEL_EXPORT.exists():
        raise FileNotFoundError(f"Model export not found at {MODEL_EXPORT}. Run 'make train' first.")
    return joblib.load(MODEL_EXPORT)


def demo_predict():
    pipe = load_pipeline()
    sample = pd.DataFrame([{"calls": 80, "minutes": 500.0, "messages": 50, "mb_used": 20000.0}])
