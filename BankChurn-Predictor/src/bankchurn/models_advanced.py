"""
Advanced model implementations for BankChurn.

Provides gradient boosting (XGBoost, LightGBM) and deep learning (PyTorch)
models with a unified factory interface. All models are sklearn-compatible
via wrappers to integrate seamlessly with existing Pipeline infrastructure.
"""
...
def build_xgboost_classifier(params: Dict[str, Any] | None = None, seed: int = 42) -> Any:
    """Build an XGBoost classifier with sensible defaults for churn prediction.
...
def build_lightgbm_classifier(params: Dict[str, Any] | None = None, seed: int = 42) -> Any:
    """Build a LightGBM classifier with sensible defaults for churn prediction.
...
if TORCH_AVAILABLE:  # pragma: no cover
    ...
    class _ChurnNet(nn.Module):
        ...
        Architecture
        ------------
        Input → BatchNorm → Linear(d,128) → ReLU → Dropout(0.3)
    ...
    class TorchTabularClassifier(BaseEstimator, ClassifierMixin):
        ...
        Implements fit/predict/predict_proba so it can be used inside
        sklearn Pipelines just like any other estimator.
        ...
        def __init__(
            ...
            self.model_: _ChurnNet | None = None
        ...
        def fit(self, X: np.ndarray, y: np.ndarray) -> "TorchTabularClassifier":
            ...
            self.model_ = _ChurnNet(input_dim, self.hidden_dims, self.dropout).to(self.device_)
            ...
            # Train/val split for early stopping (use 10% of training data)
            n = len(X_t)
            ...
            for epoch in range(self.epochs):
                ...
                self.model_.eval()
                with torch.no_grad():
                    val_X = X_t[val_idx].to(self.device_)
                    val_y = y_t[val_idx].to(self.device_)
        ...
        def predict_proba(self, X: np.ndarray) -> np.ndarray:
            ...
            self.model_.eval()
            X_t = self._to_tensor(X, dtype=torch.float32).to(self.device_)
        ...
        def _to_tensor(self, data: Any, dtype: torch.dtype) -> torch.Tensor:
            if isinstance(data, torch.Tensor):
                return data.to(dtype)
            arr = np.asarray(data, dtype=np.float32) if dtype == torch.float32 else np.asarray(data)
...
def build_model(
    ...
    ----------
    model_name : str
        One of: 'logistic_regression', 'random_forest', 'ensemble',
