...
def preprocess(
    input_csv: Path, output_csv: Path, cfg_path: Path | None = None
) -> None:
    cfg = load_config(cfg_path)
    df = pd.read_csv(input_csv)
    drops = cfg.get("data", {}).get("drop_columns", [])
    df = df.drop(
        columns=[c for c in drops if c in df.columns], errors="ignore"
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--config", default="configs/config.yaml", help="Config YAML"
    )
    args = ap.parse_args()
