...
def test_trainer_load_and_prepare(config, full_sample_data, tmp_path):
...
def test_trainer_build_pipeline(config, full_sample_data):
...
def test_trainer_with_different_random_states(config, full_sample_data):
    """Test trainer produces different results with different seeds."""
    X, y = ChurnTrainer(config).prepare_features(full_sample_data)

    # Train with seed 42
    trainer1 = ChurnTrainer(config, random_state=42)
    ...
    # Train with seed 123
    trainer2 = ChurnTrainer(config, random_state=123)
...
def test_trainer_handles_missing_columns(config, tmp_path):
...
def test_trainer_preprocessor_feature_detection(config):
...
def test_trainer_model_has_required_methods(config):
