...
SERVICES=(
  ...
  "NLPInsight-Customer-Intelligence:ml-portfolio/nlpinsight-intelligence"
)
...
aws s3 cp NLPInsight-Customer-Intelligence/models/model.joblib \
  s3://${MODELS_BUCKET}/nlpinsight/model.joblib
...
aws s3 cp NLPInsight-Customer-Intelligence/data/raw/WA_Fn-UseC_-Telco-Customer-Churn.csv \
  s3://${DATASETS_BUCKET}/nlpinsight/v1/WA_Fn-UseC_-Telco-Customer-Churn.csv
...
# Kubernetes manifests only (includes AWS overlays)
...
| Test | Type | AWS |
|------|------|-----|
| `terraform fmt` | Hard gate | ✅ Pass |
| `terraform validate` | Hard gate | ✅ Pass |
| `tfsec` | Advisory | ✅ (84/116 passed) |
| `checkov` | Advisory | ✅ (84/116 passed) |
| YAML syntax (incl. overlays) | Hard gate | ✅ 24/24 files |
| `kube-linter` | Advisory | ✅ 17 findings |
| `conftest` (OPA) | Hard gate | ✅ 0 violations |

> **Note**: tfsec/checkov advisory findings are documented trade-offs in `.tfsec.yml`. Security hardening includes: EKS restricted CIDRs, private endpoint, S3 KMS encryption, public access blocks, ECR immutable tags, RDS deletion protection + IAM auth.
...
| NLPInsight | `ml-portfolio-datasets-production` | `nlpinsight/v1/WA_Fn-UseC_-...csv` | `/data/raw/WA_Fn-UseC_-...csv` |
...
| NLPInsight | 384Mi / 200m | 768Mi / 800m | 75% | 1–3 |
...
for SVC in bankchurn-service carvision-service nlpinsight-service; do
  echo "Testing ${SVC}..."
