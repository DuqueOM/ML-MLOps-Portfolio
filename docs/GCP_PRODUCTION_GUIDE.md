...
for PROJECT in BankChurn-Predictor CarVision-Market-Intelligence NLPInsight-Customer-Intelligence; do
...
# Upload NLPInsight model
gsutil cp NLPInsight-Customer-Intelligence/models/model.joblib \
  gs://${MODELS_BUCKET}/nlpinsight/model.joblib
...
# Upload NLPInsight dataset
gsutil cp NLPInsight-Customer-Intelligence/data/raw/WA_Fn-UseC_-Telco-Customer-Churn.csv \
  gs://${DATASETS_BUCKET}/nlpinsight/v1/WA_Fn-UseC_-Telco-Customer-Churn.csv
...
sed -i "s|image: .*nlpinsight.*|image: ${AR_REPO}/nlpinsight-analyzer:latest|" k8s/nlpinsight-deployment.yaml
```
...
kubectl apply -f k8s/nlpinsight-deployment.yaml
...
kubectl rollout status deployment/nlpinsight-intelligence -n ml-portfolio --timeout=300s
```
...
kubectl exec -n ml-portfolio deployment/nlpinsight-intelligence -- curl -s http://localhost:8000/health
```
...
|-------|--------|------|
| Prediction Rate — All Services | `sum(rate(bankchurn_predictions_total[5m]))`, `sum(rate(carvision_requests_total[5m]))`, `sum(rate(nlpinsight_requests_total[5m]))` | Time series |
...
| Total Requests — NLPInsight | `sum(nlpinsight_requests_total)` | Stat |
...
- `nlpinsight-intelligence` — scrapes pods with label `app: nlpinsight-intelligence`
...
#### Quick one-shot validation (via wrapper)

```bash
# Wrapper delegates to Locust automatically
python scripts/load_test_services.py --users 10 --time 30s
```

#### Tier 3 — Infrastructure Tests (Terraform + Kubernetes)

```bash
# Run ALL infra tests (terraform fmt/validate/tfsec/checkov + kube-linter + conftest)
...
bash tests/infra/kubernetes/test_kubernetes.sh

# Skip smoke tests (offline validation only)
bash tests/infra/run_all_tests.sh --skip-smoke
```

**Infrastructure test results (current)**:
...
| YAML syntax | Hard gate | ✅ 24/24 files | — |
| `kube-linter` | Advisory | ✅ 17 findings | — |
| `conftest` (OPA) | Hard gate | ✅ 0 violations | — |

> **Note**: tfsec/checkov findings are accepted trade-offs documented in `.tfsec.yml`. They are advisory — they don't block CI.
...
1. Detects which services changed (BankChurn, CarVision, NLPInsight)
...
- **Manual**: `workflow_dispatch` with service selection (all/bankchurn/carvision/nlpinsight)
...
kubectl get pods -n ml-portfolio
# Expected: 6 pods (bankchurn, carvision, nlpinsight, mlflow, prometheus, grafana) all Running

# 2. Health checks pass (via kubectl exec)
for DEPLOY in bankchurn-predictor carvision-intelligence nlpinsight-intelligence; do
  echo "Testing ${DEPLOY}..."
...
kubectl port-forward svc/grafana-service 3000:3000 -n ml-portfolio &
# Open http://localhost:3000 (admin / MLPortfolio2026!)
...
# nlpinsight-dataset-config     3      Xs
```
...
| NLPInsight | `WA_Fn-UseC_-Telco-Customer-Churn.csv` | `nlpinsight/v1/WA_Fn-UseC_-...csv` | ~977 KB |
...
| NLPInsight | ~140Mi / 4m CPU | 384Mi / 200m | 768Mi / 800m | 36% |
...
# nlpinsight-hpa      Deployment/nlpinsight-intelligence    cpu: 2%/75%   1        3        1
```
...
nlpinsight-intelligence-788c44bcc-pxk49      1/1     Running   2m
