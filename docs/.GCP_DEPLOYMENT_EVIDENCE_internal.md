...
## 🎯 Selección Curada: Evidencia de Mayor Impacto para Documentación Pública

> **Propósito**: De las 168+ capturas de esta guía, esta sección identifica las piezas de evidencia que deben aparecer en la documentación visible del repositorio. Un reclutador dedica **< 60 segundos** por portafolio — cada imagen debe justificar su presencia.
>
> **Principio**: 1 GIF > 5 screenshots > 1 párrafo de texto.
>
> **Convención de rutas**: GCP → `docs/media/screenshots/{carpeta}/{archivo}`, AWS → `docs/media/screenshots/aws-{carpeta}/{archivo}` (prefijo `A` en los IDs).

### Tier 1 — README Principal (`README.md`) — Máximo 8 imágenes + 1 GIF + 1 video

| Prioridad | ID | Ruta relativa (desde docs/) | Qué demuestra | Dónde usarla |
|:---------:|:--:|---------------------------|---------------|--------------|
| ⭐ | G01 | `media/gifs/01-demo-prediccion.gif` | **Predicción ML en vivo** — request → modelo → response con SHAP | Hero del README |
| ⭐ | G05 | `media/screenshots/gcp-console/05-gke-workloads-running.png` | **6 workloads running en GKE** | Sección "Live Deployment — GCP" |
| ⭐ | A05 | `media/screenshots/aws-console/A05-eks-workloads-running.png` | **6 workloads running en EKS** — paridad multi-cloud | Sección "Live Deployment — AWS" |
| ⭐ | G46 | `media/screenshots/cicd/46-workflow-completado.png` | **CI/CD pipeline verde** | Sección "CI/CD" |
| ⭐ | G53 | `media/screenshots/terraform/53-terraform-plan-no-changes.png` | **Terraform plan: 0 changes** — IaC sincronizado | Sección "Infrastructure as Code" |
| ⭐ | G70 | `media/screenshots/monitoring/70-grafana-ml-dashboard-full.png` | **Grafana — 4 señales de oro** | Sección "Monitoring" |
| ⭐ | G68 | `media/screenshots/cicd/68-codecov-dashboard.png` | **~92% coverage (Codecov)** | Sección "Testing" |
| ⭐ | MC1 | `media/gifs/mc01-multi-cloud-side-by-side.gif` | **GKE vs EKS side-by-side** — mismo sistema en 2 clouds | Sección "Multi-Cloud" |
| 🎬 | — | `media/video/portfolio-demo.mp4` | **Video demo 4 min** — YouTube (no listado) | Después del hero |

### Tier 2 — READMEs de Proyecto — 3-4 imágenes por proyecto

**BankChurn-Predictor/README.md:**

| ID | Ruta relativa (desde docs/) | Qué demuestra |
|:--:|---------------------------|---------------|
| G25 | `media/screenshots/apis/25-fastapi-swagger-bankchurn.png` | Swagger UI |
| G82 | `media/screenshots/apis/82-shap-prediction-response.png` | SHAP feature contributions |
| G26 | `media/screenshots/apis/26-bankchurn-prediccion-real.png` | Predicción real + risk level |

**CarVision-Market-Intelligence/README.md:**

| ID | Ruta relativa (desde docs/) | Qué demuestra |
|:--:|---------------------------|---------------|
| G27 | `media/screenshots/apis/27-fastapi-swagger-carvision.png` | Swagger UI |
| G78 | `media/screenshots/apis/78-streamlit-data-explorer.png` | Streamlit Data Explorer |
| G79 | `media/screenshots/apis/79-streamlit-prediction.png` | Predicción de precio en vivo |

**NLPInsight-Analyzer/README.md:**

| ID | Ruta relativa (desde docs/) | Qué demuestra |
|:--:|---------------------------|---------------|
| G29 | `media/screenshots/apis/29-fastapi-swagger-nlpinsight.png` | Swagger UI |
| G84 | `media/screenshots/monitoring/84-drift-detection-output.png` | Drift detection (KS + PSI) |

### Tier 3 — GitHub Pages (`docs/`) — Máximo 25 imágenes (GCP + AWS)

| Página docs/ | GCP | AWS (equivalente) | Justificación |
|-------------|-----|-------------------|---------------|
| **architecture/infrastructure** | G05 (GKE workloads), G08 (Ingress IP), G09 (Artifact Registry) | A05 (EKS workloads), A08 (ALB DNS), A09 (ECR) | Infra real desplegada en ambos clouds |
| **architecture/overview** | G17 (kubectl pods) | A17 (kubectl pods EKS) | Estado del sistema |
| **DEPLOYMENT_EVIDENCE** | G05, G53, G70 | A05, A53, A70 | Paridad multi-cloud |
| **operations/monitoring** | G34 (Grafana), G37 (Prometheus targets), G71 (P95 latency) | A34, A37 | Observabilidad |
| **models/catalog** | G39 (MLflow experiments), G55 (XGBoost comparison) | — (misma instancia) | MLflow tracking |
| **architecture/cicd** | G46 (workflow), G68 (Codecov), G86 (drift workflow) | — (mismo pipeline) | CI/CD |
| **architecture/infrastructure** | G22b (infra test results) | A22b (infra test AWS) | Testing de IaC (tfsec, checkov, kube-linter, OPA) |
| **FEATURES** | G82 (SHAP), G78-G81 (Streamlit tabs), G84 (drift) | — | Features avanzadas |

### Reglas de Oro

1. **Máximo 8 imágenes por README** — Incluye GCP + AWS side-by-side
2. **GIFs > Screenshots** — 1 GIF de 10s = 3 screenshots
3. **Cada imagen necesita caption** — Sin contexto es ruido
4. **AWS como par de GCP** — Para cada captura GCP (G##) debe existir su equivalente AWS (A##)
5. **Orden narrativo** — Desplegado → API → Predicción → Monitoreo → CI/CD
...
Un **Pod** es la unidad mínima de Kubernetes. Contiene uno o más contenedores Docker corriendo juntos. En tu proyecto, cada API (BankChurn, CarVision, NLPInsight) corre en su propio Pod. Cuando ves `kubectl get pods` y todos dicen `Running`, significa que todas tus aplicaciones están vivas y respondiendo.

### ¿Qué es el Ingress?

El **Ingress** es el "portero" de tu cluster. Es un balanceador de carga que recibe tráfico de internet y lo dirige al servicio correcto según la URL. Tu Ingress tiene la IP pública `34.120.120.57` y dirige `/bankchurn/*` a la API de BankChurn, `/carvision/*` a la API de CarVision, y `/nlpinsight/*` a la API de NLPInsight.
...
2. Verás la lista de todos los Deployments. Deben aparecer:
   ...
   - `nlpinsight-analyzer` — API de análisis de sentimiento financiero (NLP)
...
> - **Qué debe verse**: Lista de servicios: bankchurn-service, carvision-service (puertos 80 + 8501), nlpinsight-service, grafana-service, etc.
...
5. Verás las 3 imágenes: `bankchurn-predictor`, `carvision-market-intelligence`, `nlpinsight-customer-intelligence`
...
5. Verás las carpetas: `bankchurn/`, `carvision/`, `nlpinsight/`
...
7. Haz clic en el bucket de **modelos** (`*-ml-models-production`) y verás las carpetas: `bankchurn/`, `carvision/`, `nlpinsight/`
...
9. Verás las carpetas: `bankchurn/`, `carvision/`, `nlpinsight/`, cada una con subcarpeta `v1/`
...
> - **Qué debe verse**: El bucket de datasets con carpetas `bankchurn/v1/`, `carvision/v1/`, `nlpinsight/v1/` y los archivos CSV con sus tamaños
...
nlpinsight-analyzer-xxxx-xxxx          1/1     Running   0          Xh
...
gs://ml-portfolio-duque-om-202602-ml-models-production/nlpinsight/model.joblib
...
gs://ml-portfolio-duque-om-202602-datasets-production/nlpinsight/v1/financial_phrasebank.csv
```
...
### 5.6b — Infrastructure Tests (Terraform + Kubernetes Validation)

**¿Qué son los infrastructure tests?** Son validaciones automatizadas que verifican que tu código de infraestructura (Terraform) y tus manifiestos de Kubernetes cumplen con estándares de formato, seguridad y mejores prácticas. En empresas serias, estos tests son parte del CI/CD — no puedes hacer merge si la infra no pasa validación.

**¿Por qué importa para el portafolio?** Muy pocos ingenieros junior/mid testean su infraestructura. Mostrar que tienes un suite de tests para Terraform y Kubernetes te diferencia inmediatamente. Un reclutador técnico verá: tfsec, checkov, kube-linter, conftest/OPA — herramientas enterprise.
...
# Kubernetes: YAML syntax + kube-linter + conftest (OPA policies)
bash tests/infra/kubernetes/test_kubernetes.sh
```

**¿Qué verás?** Un reporte con resultados por categoría:
```
═══════════════════════════════════════════
  Infrastructure Tests — ML-MLOps Portfolio
═══════════════════════════════════════════

TERRAFORM TESTS (GCP):
  ✅ terraform fmt     — PASS (hard gate)
  ✅ terraform validate — PASS (hard gate)
  ⚠️  tfsec            — 51/71 passed (advisory)
  ⚠️  checkov          — 51/71 passed (advisory)

KUBERNETES TESTS:
  ✅ YAML syntax       — 24/24 files valid
  ⚠️  kube-linter      — 17 findings (advisory, tuned config)
  ✅ conftest (OPA)    — 0 violations (13 custom rules)

SUMMARY: All hard gates PASSED. Advisory findings documented.
```

---

> **📸 CAPTURA #22b — Infrastructure Test Results**
>
> - **Archivo**: `docs/media/screenshots/terminal/22b-infra-test-results.png`
> - **Qué debe verse**: Output completo del test runner con ✅ para hard gates y ⚠️ para advisory findings
> - **Por qué importa**: Demuestra testing automatizado de IaC — diferenciador clave vs portfolios que solo despliegan sin validar. Muestra conocimiento de tfsec, checkov, kube-linter, OPA/Rego
> - **Dónde usarla**: `docs/architecture/infrastructure.md` (sección Testing), `DEPLOYMENT_EVIDENCE.md`
> - **Tip**: Captura la terminal completa con los resultados de GCP + AWS para mostrar paridad multi-cloud
...
# Health check NLPInsight
echo "=== NLPInsight Health ==="
kubectl exec -n ml-portfolio deployment/nlpinsight-analyzer -- \
  curl -s http://localhost:8000/health | python3 -m json.tool
```
...
### 6.5 — Port-Forward y Swagger de NLPInsight
...
> **📸 CAPTURA #29 — FastAPI Swagger UI de NLPInsight**
>
> - **Archivo**: `docs/media/screenshots/apis/29-fastapi-swagger-nlpinsight.png`
> - **URL**: `http://localhost:8003/docs`
> - **Qué debe verse**: La interfaz Swagger de NLPInsight con los endpoints de análisis de sentimiento financiero (NLP)
> - **Por qué importa**: Completa la evidencia de los 3 servicios ML independientes

**Hacer una predicción de NLPInsight:**
```bash
...
> **📸 CAPTURA #30 — Predicción Real de NLPInsight**
>
> - **Archivo**: `docs/media/screenshots/apis/30-nlpinsight-prediccion-real.png`
> - **Qué debe verse**: La respuesta JSON con la probabilidad de sentimiento del texto financiero
> - **Por qué importa**: Demuestra el tercer caso de uso ML — análisis de sentimiento financiero con NLP
...
1. Abre 3 pestañas en el navegador:
   ...
   - Pestaña 3: `http://localhost:8003/docs` (NLPInsight)
...
4. El dashboard tiene **10 paneles** que monitorean los 3 servicios ML:
   - **Prediction Rate — All Services**: BankChurn, CarVision y NLPInsight req/s
...
> **Nota**: La captura principal del dashboard es la **#38d** (sección 7.3) — muestra el dashboard poblado con métricas reales después del load test.
...
4. Los jobs visibles son: `bankchurn-predictor`, `carvision-intelligence`, `nlpinsight-analyzer`, `prometheus`
...
> | `nlpinsight-analyzer` | 1–3 | HPA con CPU 75% (escala con tráfico de inferencia) |
...
| **NLPInsight** | ~140Mi / 4m CPU | 384Mi / 200m | 768Mi / 800m | 36% mem | 64% |
...
| Config | BankChurn | CarVision | NLPInsight |
|--------|-----------|-----------|-----------|
...
**¿Por qué CPU-only y no CPU + memoria?** Los servicios ML cargan el modelo completo en RAM al iniciar (~300Mi para BankChurn, ~550Mi para CarVision, ~140Mi para NLPInsight). Esta memoria es **fija** — no varía con el tráfico. El HPA calcula `desiredReplicas = ceil(currentReplicas × usage/target)`. Con memoria fija al 67%, si hay 3 réplicas: `ceil(3 × 67/80) = ceil(2.51) = 3` — **nunca puede bajar**. Cada pod replica el mismo footprint de modelo, así que agregar réplicas no reduce la memoria por pod. CPU sí correlaciona con tráfico de inferencia y es la métrica correcta para escalar servicios ML.
...
- `k8s/nlpinsight-deployment.yaml` — HPA CPU-only con behavior definido
...
```
  ...
  NLPInsight-Analyzer    300   300   0.0%   299ms   265ms   570ms   728ms

  SLA Compliance:
    ⚠️  BankChurn-Predictor: P95 1675ms > 500ms (normal — ensemble 5 modelos con SHAP)
    ⚠️  CarVision/NLPInsight: P95 ~600ms bajo concurrency 10 (dentro de SLA con concurrency ≤5)
```
...
| Métrica | BankChurn | CarVision | NLPInsight |
|---------|-----------|-----------|-----------|
...
| `nlpinsight-analyzer` | 4m | 134Mi | 134/384Mi = 35% | 65% |
| `grafana` | 2m | 100Mi | — | — |
| `prometheus` | 3m | 40Mi | — | — |
...
> - **Qué debe verse**: La interfaz de MLflow con los experimentos de entrenamiento (BankChurn, CarVision, NLPInsight)
...
#### Paso 1 — Ejecutar los 9 Experimentos MLflow

Los experimentos ya están implementados en `scripts/run_experiments.py`. Este script entrena 3 modelos por proyecto (9 runs en total) y registra todo en MLflow automáticamente.

```bash
# Prerrequisito: port-forward al MLflow server en GKE
kubectl port-forward svc/mlflow-service 5000:5000 -n ml-portfolio

# Ejecutar los 9 experimentos (desde la raíz del repo)
MLFLOW_TRACKING_URI=http://localhost:5000 python scripts/run_experiments.py
```

**Experimentos que se crean:**

| Experimento MLflow | Run | Modelo | Propósito |
|---|---|---|---|
| `BankChurn-Predictor` | BC-1_Baseline | LogisticRegression | Línea base simple |
| `BankChurn-Predictor` | BC-2_RandomForest_Tuned | RandomForest (balanced) | Modelo optimizado ⭐ |
| `BankChurn-Predictor` | BC-3_Overfit_Demo | RandomForest (sin regularización) | Demostrar overfitting |
| `CarVision-Market-Intelligence` | CV-1_Baseline | RandomForest | Línea base |
| `CarVision-Market-Intelligence` | CV-2_Tuned | RandomForest (tuned) | Modelo optimizado ⭐ |
| `CarVision-Market-Intelligence` | CV-3_GradientBoosting | GradientBoosting | Comparación de algoritmo |
| `NLPInsight-Analyzer` | TC-1_Baseline | LogisticRegression | Línea base |
| `NLPInsight-Analyzer` | TC-2_RandomForest_Tuned | RandomForest (balanced) | Modelo optimizado ⭐ |
| `NLPInsight-Analyzer` | TC-3_Overfit_Demo | RandomForest (sin regularización) | Demostrar overfitting |

Cada run registra automáticamente: parámetros del modelo, métricas (accuracy, F1, precision, recall, AUC), tags (framework, project, run_type), y dataset de entrada.
...
**Paso 2a — Comparar los 3 runs de BankChurn:**
1. En MLflow UI, haz clic en el experimento **`BankChurn-Predictor`** en la barra lateral izquierda
2. Verás 3 runs: BC-1_Baseline, BC-2_RandomForest_Tuned, BC-3_Overfit_Demo
3. Selecciona los 3 runs (checkbox a la izquierda de cada uno)
4. Haz clic en el botón **"Compare"** (arriba de la lista)
5. Verás una tabla comparativa con parámetros y métricas lado a lado

---

> **📸 CAPTURA #55 — MLflow: Comparación de 3 Experimentos BankChurn**
>
> - **Archivo**: `docs/media/screenshots/monitoring/55-mlflow-xgboost-comparison.png`
> - **URL**: `http://localhost:5000/#/experiments/<ID>/compare`
> - **Qué debe verse**: Tabla comparativa de los 3 runs de BankChurn con columnas de parámetros y métricas (accuracy, f1, recall, auc). El run "Tuned" debe tener mejores métricas que Baseline, y "Overfit" debe mostrar train_accuracy alta pero test_accuracy más baja
> - **Por qué importa**: Demuestra experimentación sistemática — baseline → optimización → análisis de overfitting

---

**Paso 2b — Gráfica de Parallel Coordinates:**
1. En la vista de comparación (después de hacer Compare), busca la pestaña **"Parallel Coordinates"** en la parte superior
2. En el menú de parámetros/métricas, selecciona `test_roc_auc` o `test_f1` como métrica final
3. Verás líneas de colores mostrando cómo cada configuración afecta la métrica
...
> - **Qué debe verse**: Gráfico de coordenadas paralelas mostrando la relación entre parámetros (n_estimators, max_depth, etc.) y métricas, con líneas de colores indicando el rendimiento
> - **Por qué importa**: Visualización profesional de búsqueda de hiperparámetros

---

**Paso 2c — Comparar los mejores runs de los 3 proyectos (cross-project):**
1. Vuelve a la página principal de MLflow (clic en "MLflow" arriba a la izquierda)
2. Abre el experimento `BankChurn-Predictor`, haz clic en el run `BC-2_RandomForest_Tuned` y copia su Run ID
3. Repite para `CarVision-Market-Intelligence` → `CV-2_Tuned` y `NLPInsight-Analyzer` → `TC-2_RandomForest_Tuned`
4. Usa la búsqueda o abre cada run individualmente para comparar métricas

---

> **📸 CAPTURA #57 — MLflow: Comparación Cross-Project (BankChurn vs CarVision vs NLPInsight)**
>
> - **Archivo**: `docs/media/screenshots/monitoring/57-mlflow-cross-model-comparison.png`
> - **URL**: `http://localhost:5000` — vista de los 3 experimentos
> - **Qué debe verse**: Los mejores runs de cada proyecto con sus métricas principales lado a lado (BankChurn AUC 0.87, CarVision R² 0.77, NLPInsight AUC 0.84)
> - **Por qué importa**: Demuestra que gestionas múltiples proyectos ML con tracking centralizado

---

**Paso 2d — Capturar detalle del mejor run:**
1. En el experimento `BankChurn-Predictor`, haz clic en el run `BC-2_RandomForest_Tuned`
2. Verás la página de detalle con: parámetros, métricas, tags, y dataset

> **Nota**: La captura #40 (`40-mlflow-run-detalle.png`) ya cubre este paso. Verifica que muestra correctamente las 7 métricas, 4 tags, y el dataset.

---

**Paso 2e — Scatter Plot (opcional):**
1. En la vista de comparación de BankChurn (3 runs seleccionados → Compare)
2. Busca la pestaña **"Scatter Plot"**
3. Eje X: `test_recall`, Eje Y: `test_precision`
...
> - **Qué debe verse**: Scatter plot con 3 puntos (uno por run) mostrando el trade-off recall/precision
> - **Por qué importa**: Visualiza el trade-off fundamental en clasificación

---

#### Paso 3 — Verificar los 3 Experimentos

Después de tomar las capturas, verifica que todo esté registrado correctamente:

```bash
# Verificar que los 3 experimentos existen y tienen runs
curl -s http://localhost:5000/api/2.0/mlflow/experiments/search | python -m json.tool | grep "name"
# Debe mostrar: BankChurn-Predictor, CarVision-Market-Intelligence, NLPInsight-Analyzer
```

> 💡 **Tip para entrevistas**: "Ejecuté 9 experimentos trackeados en MLflow (3 por proyecto), comparando modelos baseline vs optimizados vs overfitted para demostrar el impacto del tuning de hiperparámetros. El modelo optimizado de BankChurn logró AUC 0.87 con SHAP explainability y drift detection automatizado."
...
> kubectl port-forward svc/nlpinsight-service 8003:8000 -n ml-portfolio &
> kubectl port-forward svc/grafana-service 3000:3000 -n ml-portfolio &
> kubectl port-forward svc/prometheus-service 9090:9090 -n ml-portfolio &
> ```
...
> **Nota**: La captura del dashboard completo es la **#38d** (sección 7.3) — ya cubre esta vista con datos reales.
...
> - **URL**: `http://localhost:3000/d/ml-portfolio` (panel expandido)
> - **Qué debe verse**: Panel de Error Rate expandido. Si la gráfica está **vacía o muestra 0%**, eso es correcto — significa que no hay errores HTTP 4xx/5xx. La query PromQL solo genera datos cuando existen requests con status de error, y si todos los requests son `status="200"`, el panel queda vacío (= 0% error rate ✅)
> - **Por qué importa**: Error rate es una de las 4 señales de oro — un panel vacío aquí es la **mejor evidencia posible** de que tus APIs son confiables

---

> **Nota**: La captura del datasource Prometheus ya está cubierta por la **#35** (sección 6.3).
...
> **Nota**: La captura de Prometheus Targets ya está cubierta por la **#37** (sección 6.4).

**Paso 3 — Métricas raw del endpoint /metrics:**

Abre directamente el endpoint de métricas de una API y filtra las métricas custom:

```bash
# Mostrar SOLO las métricas custom de BankChurn (ignorar métricas genéricas de Python/proceso)
curl -s http://localhost:8001/metrics | grep -E "^(#.*bankchurn|bankchurn)"
```

> ⚠️ **Importante**: NO uses `head -30` — las primeras 30 líneas son métricas genéricas de Python GC y proceso. Las métricas custom `bankchurn_*` aparecen más abajo en el output. Usa `grep bankchurn` para filtrarlas.
...
> - **Captura de**: Terminal
> - **Qué debe verse**: Output de `curl ... | grep bankchurn` mostrando métricas custom: `bankchurn_requests_total{endpoint="/predict",method="POST",status="200"}`, `bankchurn_request_duration_seconds_count`, `bankchurn_predictions_total{risk_level="HIGH/MEDIUM/LOW"}`, etc.
> - **Por qué importa**: Demuestra que implementaste instrumentación Prometheus custom en el código de las APIs — contadores de predicciones por risk_level, histogramas de latencia, etc. (no solo métricas genéricas de Python)
...
| NLPInsight-Analyzer/src/nlpinsight | 352 | 336 | 16 | **95.45%** |
...
> - **URL**: `https://app.codecov.io/gh/DuqueOM/ML-MLOps-Portfolio`
> - **Qué debe verse**: Dashboard de Codecov mostrando ~92% coverage total, gráfica de evolución temporal (nov 2025 → feb 2026), donut chart por proyecto, y tabla con BankChurn 88%, CarVision 95%, NLPInsight 95%
...
> 💡 **Tip para entrevistas**: "Mi portafolio tiene ~92% de test coverage verificado por Codecov, integrado en el CI pipeline de GitHub Actions. Cada push genera un reporte de coverage que se sube automáticamente. BankChurn tiene 88%, CarVision 95%, y NLPInsight 95%. El CLI tiene 97% y models_advanced 100%."
...
# NLPInsight — versionar el dataset financiero (Financial PhraseBank)
dvc add NLPInsight-Analyzer/data/raw/financial_phrasebank.csv
```
...
dvc add NLPInsight-Analyzer/models/model.joblib
...
# ./NLPInsight-Analyzer/data/raw/financial_phrasebank.csv.dvc
# ./NLPInsight-Analyzer/models/model.joblib.dvc
```
...
- Track 3 raw datasets (BankChurn, CarVision, NLPInsight)
...
**Narración**:
> *"Hola, en este video voy a mostrar el deployment en producción de mi portafolio de MLOps en Google Cloud Platform. El sistema consiste en tres APIs de Machine Learning — predicción de churn bancario, valoración de vehículos, y análisis de sentimiento financiero con NLP — todas corriendo simultáneamente en un cluster de Kubernetes en GCP, con monitoreo en tiempo real, tracking de experimentos con MLflow, y un pipeline de CI/CD con GitHub Actions. Toda la infraestructura fue creada con Terraform como Infrastructure as Code."*
...
**Narración**:
> *"Aquí vemos la consola de GCP. En Kubernetes Engine, los seis workloads están corriendo: las tres APIs de ML, MLflow, Prometheus y Grafana. Cada uno tiene su imagen Docker almacenada en Artifact Registry — bankchurn-predictor, carvision-market-intelligence, y nlpinsight-customer-intelligence, todas con sus tags de versión."*
...
> - **Qué debe verse**: Lista de 6 Deployments en namespace `ml-portfolio`, todos con status **Ready** y los pods AVAILABLE matching DESIRED (1/1 cada uno): bankchurn-predictor, carvision-market-intelligence, nlpinsight-customer-intelligence, mlflow-server, prometheus, grafana
...
> - **URL**: `console.aws.amazon.com/eks/home?region=us-east-1#/clusters/ml-portfolio-eks-production?selectedTab=cluster-resources-tab`
> - **Qué debe verse**: Lista de Services en namespace `ml-portfolio`: bankchurn-service (ClusterIP, port 80), carvision-service, nlpinsight-service, mlflow-service, prometheus-service, grafana-service
...
> - **URL**: `console.aws.amazon.com/eks/home?region=us-east-1#/clusters/ml-portfolio-eks-production?selectedTab=cluster-resources-tab`
> - **Qué debe verse**: Ingress resource mostrando el ADDRESS con el DNS del ALB (algo como `k8s-mlportf-mlportf-XXXXXXXXXX-XXXXXXXXXX.us-east-1.elb.amazonaws.com`), las rules de routing hacia cada servicio (/bankchurn, /carvision, /nlpinsight)
...
> - **Qué debe verse**: 3 repositorios: `ml-portfolio/bankchurn-predictor`, `ml-portfolio/carvision-intelligence`, `ml-portfolio/nlpinsight-analyzer`, cada uno con su URI, fecha de creación, y estado de encryption (AES256)
...
> - **Qué debe verse**: Estructura de carpetas: `bankchurn/model.joblib` (4 MB), `carvision/model.joblib` (6 KB), `nlpinsight/model.joblib` (156 KB). Cada archivo con su fecha de upload, storage class, y encryption status
...
nlpinsight-customer-intel-xxxx-xxxx      1/1     Running   0          Xh      10.0.1.xx    ip-10-0-1-xx.ec2.internal
...
### 19.6 — FastAPI Swagger UI: NLPInsight en EKS

**Paso a paso:**
1. Abre `http://localhost:8003/docs`
2. Verás la documentación Swagger de NLPInsight

---

> **📸 CAPTURA #A30 — FastAPI Swagger: NLPInsight en EKS**
>
> - **Archivo**: `docs/media/screenshots/aws-apis/A30-fastapi-swagger-nlpinsight-eks.png`
> - **URL**: `http://localhost:8003/docs`
> - **Qué debe verse**: Swagger UI de NLPInsight con los endpoints de análisis de sentimiento financiero (NLP)
> - **Por qué importa**: Tercer proyecto completando el trío de APIs en EKS

---

### 19.7 — Predicción Real: NLPInsight en EKS

**Paso a paso:**
1. En Swagger UI de NLPInsight, haz clic en `POST /predict`
2. Pega el JSON de un texto financiero:
...
> **📸 CAPTURA #A31 — Predicción Real: NLPInsight en EKS**
>
> - **Archivo**: `docs/media/screenshots/aws-apis/A31-nlpinsight-prediccion-real-eks.png`
...
echo ""
echo "=== NLPInsight Prediction (EKS) ==="
curl -s -X POST http://localhost:8003/predict \
  -H "Content-Type: application/json" \
...
> - **URL**: `http://localhost:9090/targets`
> - **Qué debe verse**: Todos los targets en estado **UP** (verde): bankchurn-predictor, carvision-service, nlpinsight-service, con sus endpoints, last scrape time, y scrape duration
...
> - **Qué debe verse**: Lista de experimentos de MLflow: BankChurn, CarVision, NLPInsight, cada uno con sus runs, métricas (AUC, F1, R², RMSE), y artefactos. El backend ahora es RDS PostgreSQL + S3 (en lugar de Cloud SQL + GCS)
...
> - **Qué debe verse**: Comparación de modelos de diferentes proyectos: BankChurn vs CarVision vs NLPInsight en métricas clave
...
> - **Qué debe verse**: Output de pytest mostrando los tests pasando y coverage: BankChurn 88%, CarVision 95%, NLPInsight 95%. Los tests son los MISMOS independientemente del cloud de deployment
...
> - **Qué debe verse**: Dashboard de Codecov mostrando: Overall ~92%, BankChurn 88.26%, CarVision 95.16%, NLPInsight 95.45%. Con el gráfico de coverage over time
...
> - **Qué debe verse**: Métricas de modelos por proyecto: BankChurn (AUC, F1, Recall), CarVision (R², RMSE), NLPInsight (AUC, F1)
...
| 20-24 | Ejecutar predicción para NLPInsight |
...
| 3-5 | Título en cada panel: "BankChurn", "CarVision", "NLPInsight" |
| 5-10 | Panel 1: curl a BankChurn → response instantáneo |
| 10-15 | Panel 2: curl a CarVision → response instantáneo |
| 15-20 | Panel 3: curl a NLPInsight → response instantáneo |
...
| 25-30 | Repetir con NLPInsight en ambos lados |
...
>
> The core consists of three machine learning services: BankChurn Predictor — a customer churn model for banking with eighty-eight percent test coverage. CarVision Market Intelligence — a vehicle price prediction model with a Streamlit dashboard achieving ninety-five percent coverage. And NLPInsight Analyzer — a financial sentiment analysis model (TF-IDF + LogisticRegression) achieving ninety-five percent test coverage.
>
...
>
> You can see six workloads, all in green, all running: the three ML APIs — BankChurn, CarVision, and NLPInsight — plus MLflow for experiment tracking, Prometheus for metrics collection, and Grafana for dashboards.
>
...
>
> And NLPInsight — a fiber optic customer on a month-to-month contract. The model identifies this as a high churn risk due to the contract type and electronic check payment method.
>
...
>
> In the Resources tab, you can see the same six deployments: BankChurn, CarVision, NLPInsight, MLflow, Prometheus, and Grafana. All showing the desired count matching the available count — everything is healthy.
>
...
>
> NLPInsight on EKS: same churn risk classification.
>
...
>
> NLPInsight: both return the same classification. Identical.
>
...
>
> Three machine learning models — BankChurn, CarVision, and NLPInsight — serving real-time predictions with eighty-eight to ninety-five percent test coverage.
>
...
Se realizó una evaluación integral del portafolio simulando la perspectiva de un reclutador senior/staff con 20 años de experiencia evaluando un candidato MLOps junior. La evaluación cubrió: código, tests, infraestructura, CI/CD, observabilidad, seguridad, documentación.

**Calificación**: 7.8/10 — **Percentil 85-90** entre candidatos junior MLOps.

### Evidencia de Problemas Encontrados

| # | Problema | Archivo de Evidencia Sugerido | Tipo |
|---|----------|-------------------------------|------|
| 1 | HPA huérfano `telecom-hpa` → deployment inexistente | `media/screenshots/k8s/hpa-orphan-before.png` | Screenshot `kubectl get hpa` antes del fix |
| 2 | 2 tests NLPInsight fallando (Keras 3 + Python 3.13) | `media/screenshots/tests/nlpinsight-2-failed-before.png` | Screenshot pytest output |
| 3 | Versión inconsistente README (7.0.0 vs 6.3.0) | `media/screenshots/docs/version-mismatch.png` | Screenshot del badge vs footer |
| 4 | Sin NetworkPolicies (default-allow) | `media/screenshots/k8s/no-network-policies-before.png` | Screenshot `kubectl get netpol` vacío |
| 5 | Sin PodDisruptionBudgets | `media/screenshots/k8s/no-pdb-before.png` | Screenshot `kubectl get pdb` vacío |

### Evidencia de Fixes Aplicados

| # | Fix | Archivo de Evidencia Sugerido | Tipo |
|---|-----|-------------------------------|------|
| 1 | HPA eliminado | `media/screenshots/k8s/hpa-clean-after.png` | Screenshot `kubectl get hpa` — solo 3 HPAs |
| 2 | Tests NLPInsight verdes | `media/screenshots/tests/nlpinsight-57-passed.png` | Screenshot pytest 57 passed, 99.57% cov |
| 3 | README versión consistente | `media/screenshots/docs/version-fixed.png` | Screenshot badge + footer ambos 7.0.0 |
| 4 | NetworkPolicies aplicadas | `media/screenshots/k8s/network-policies-after.png` | Screenshot `kubectl get netpol` — 4 policies |
| 5 | PDBs aplicados | `media/screenshots/k8s/pdb-after.png` | Screenshot `kubectl get pdb` — 3 PDBs |
| 6 | Metrics-server funcional | `media/screenshots/k8s/kubectl-top-nodes.png` | Screenshot `kubectl top nodes` con datos reales |
| 7 | Tests 3 proyectos verdes | `media/screenshots/tests/all-tests-green.png` | Screenshot resumen 279 passed |

### Resumen de Cobertura Post-Fix

```
Proyecto     | Tests  | Cobertura | Threshold CI
-------------|--------|-----------|-------------
BankChurn    | 185    | 89.26%    | 79% ✅
CarVision    | 37     | 95.41%    | 80% ✅
NLPInsight   | 57+1xp | 99.57%    | 60% ✅
TOTAL        | 279+   | ~94.75%   | —
```

### Archivos Modificados/Creados

- `k8s/network-policies.yaml` — **NUEVO** — 4 NetworkPolicies (default-deny + 3 allow)
- `k8s/pod-disruption-budgets.yaml` — **NUEVO** — 3 PDBs (minAvailable=1)
- `NLPInsight-Analyzer/tests/test_training.py` — Fix Keras 3 compat (sys.modules stub)
- `README.md` — Footer versión 6.3.0 → 7.0.0
- `docs/GCP_PRODUCTION_GUIDE.md` — Encabezado español + sección v7.1.0
- `docs/AWS_PRODUCTION_GUIDE.md` — Encabezado español + sección v7.1.0

### Comandos de Verificación para Screenshots

```bash
# Estado cluster post-fix
kubectl get pods -n ml-portfolio
kubectl get hpa -n ml-portfolio
kubectl get networkpolicies -n ml-portfolio
kubectl get pdb -n ml-portfolio
kubectl top nodes
kubectl top pods -n ml-portfolio

# Tests
cd BankChurn-Predictor && python -m pytest tests/ -q --cov=src
cd CarVision-Market-Intelligence && python -m pytest tests/ -q --cov=src/carvision
cd NLPInsight-Analyzer && python -m pytest tests/ -q --cov=src/nlpinsight

# Linting
python -m black --check */src */app
python -m isort --check-only */src */app
python -m flake8 */src --max-line-length=120 --count
```
